...
$livingcondition = $previousexperience = $adoptionreason = $prevpet = "";
...
if (isset($_POST["createrequest"])) {
    ...
    } elseif (strlen($previousexperience) < 1) {
        $error = true;
    ...
    } elseif (strlen($adoptionreason) < 1) {
        $error = true;
    ...
    if (!$error) {
        ...
        if (mysqli_query($connect, $sqlAdoptionRequest)) {
            // Redirect to confirmation.php with form data as GET parameters
            header("Location: confirmation.php?id={$id}&livingcondition=" . urlencode($livingcondition) . "&previousexperience=" . urlencode($previousexperience) . "&adoptionreason=" . urlencode($adoptionreason) . "&prevpet=" . urlencode($prevpet));
            exit; // Terminate script execution after redirection
        } else {
...
<body>
    ...
    <div class="container mb-5 mt-5">
        <form method="POST" action="" enctype="multipart/form-data">
            <div class="mb-4 mt-5">
                <label for="livingcondition" class="form-label">Temperature:</label>
                <textarea type="text" style="height: 10vh;" class="form-control" id="livingcondition" aria-describedby="livingcondition" name="livingcondition" required><?= $livingcondition ?></textarea>
                <span class="text-danger"><?= $livingconditionError ?></span>
            </div>
            <div class="mb-4 mt-5">
                <label for="previousexperience" class="form-label">Do you have any kind of allergies towards pets?</label>
                <textarea type="text" style="height: 10vh;" class="form-control" id="previousexperience" aria-describedby="previousexperience" name="previousexperience" required><?= $previousexperience ?></textarea>
                <span class="text-danger"><?= $previousexperienceError ?></span>
            </div>
            <div class="mb-4 mt-5">
                <label for="adoptionreason" class="form-label">Adoption Reason:</label>
                <textarea type="text" style="height: 10vh;" class="form-control" id="adoptionreason" aria-describedby="adoptionreason" name="adoptionreason" required><?= $adoptionreason ?></textarea>
                <span class="text-danger"><?= $adoptionreasonError ?></span>
            </div>
            <div class="mb-4 mt-5">
                <label for="prevpet" class="form-label">Do you already have a pet? If yes, mention its breed:</label>
                <textarea type="text" style="height: 10vh;" class="form-control" id="prevpet" aria-describedby="prevpet" name="prevpet" required><?= $prevpet ?></textarea>
                <span class="text-danger"><?= $adoptionreasonError ?></span>
            </div>
